% 2D NxN Square Truss Problem-- Specialized for "Case 13" Unit Cell
% "Case 13" is a 2x2 unit cell, symmetric in both x and y, with all 
% possible positions for elements filled in (with the center cavity always
% having an element running from the lower-left node to the upper-right
% node.
...
UC = [2,3,4,5,6,8,10,12,14,16,18,20];
CBasket = []; volFracs = [];
for nuc = 1:1:size(UC,2)
    ...
    % Calculated Inputs
    sidenum = nucFac + 1; % Number of nodes on each size of square grid
...
% Collect plotting outputs
C11 = [];C12 = [];C33 = [];C31 = [];C32 = [];C13 = [];C23 = [];
for q = 1:1:size(CBasket,3)
    C11 = [C11,CBasket(1,1,q)];
    C12 = [C12,CBasket(1,2,q)];
    C33 = [C33,CBasket(3,3,q)];
    C31 = [C31,CBasket(3,1,q)];
    C32 = [C32,CBasket(3,2,q)];
    C13 = [C13,CBasket(1,3,q)];
    C23 = [C23,CBasket(2,3,q)];
end

% Plot C11, C12, C33 (desired) values for increasing # of unit cells
subplot(1,2,1)
...
% Plot other (undesired) C-matrix values for increasing # of unit cells
subplot(1,2,2)
plot(UC.^2,C31,'b*',UC.^2,C32,'r*',UC.^2,C13,'g*',UC.^2,C23,'c*')
xlabel('Number of Unit Cells');
ylabel('C-Value (Pa)');
legend('C31','C32','C13','C23');
axis([0 (UC(end)^2+10) 0 6]);
...
% FUNCTION TO GENERATE "CASE-13"-SPECIFIC ELEMENT CONNECTIVITY ARRAY
function CA = generateCA(nucFac,sidenum)
    ...
    if nucFac > 1
        for j = 1:1:(nucFac-1)
            ...
            for i = (j+1):sidenum:((sidenum^2)-(2*sidenum)+1+j)
                CA = [CA;i,(i+sidenum)];
            end
            % Vertical divisions
            for i = ((j*sidenum)+1):1:((j*sidenum)+sidenum-1)
                CA = [CA;i,(i+1)];
            end
        end
    end
    
    % Interior diagonal elements 
    for j = 1:1:nucFac
        for i = j:sidenum:((sidenum^2)-(2*sidenum)+j)
            CA = [CA;i,(i+sidenum+1)];
        end
    end
end
