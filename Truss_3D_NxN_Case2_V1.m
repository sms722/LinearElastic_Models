% 3D NxNxN Cubic Truss Problem-- Specialized for "Case 2-3D" Unit Cell
% "Case 2-3D" is a 3x3x3 unit cell, symmetric in x, y, and z
...
for nuc = 1:1:size(UC,2)
    ...
    C = [];
    [C,uBasket,FBasket,K] = generateC(sel,NC,CA,A,E,C);
    C = C./nucFac;
...
subplot(2,2,1)
plot(UC.^3,C11,'b*',UC.^3,C12,'r*',UC.^3,C32,'g*',...
...
function CA = generateCA(nucFac,sidenum)
    ...
    % Create diamonds around yz-plane face-center nodes
    for q = 1:1:size(fcn_yz,2)
        CA = [CA;fcn_yz(q)-1,fcn_yz(q)-sidenum;
                 fcn_yz(q)-1,fcn_yz(q)+sidenum;
                 fcn_yz(q)+1,fcn_yz(q)-sidenum;
                 fcn_yz(q)+1,fcn_yz(q)+sidenum];
    end
    
    % Create diamonds around xz-plane face-center nodes
    for q = 1:1:size(fcn_xz,2)
        CA = [CA;fcn_xz(q)-1,fcn_xz(q)-(sidenum^2);
                 fcn_xz(q)-1,fcn_xz(q)+(sidenum^2);
                 fcn_xz(q)+1,fcn_xz(q)-(sidenum^2);
                 fcn_xz(q)+1,fcn_xz(q)+(sidenum^2)];
    end
    
    % Create diamonds around xy-plane face-center nodes
    for q = 1:1:size(fcn_xy,2)
        CA = [CA;fcn_xy(q)-sidenum,fcn_xy(q)-(sidenum^2);
                 fcn_xy(q)-sidenum,fcn_xy(q)+(sidenum^2);
                 fcn_xy(q)+sidenum,fcn_xy(q)-(sidenum^2);
                 fcn_xy(q)+sidenum,fcn_xy(q)+(sidenum^2)];
    end
    
    % Create octahedrons around center-center nodes
    for q = 1:1:size(ccn,2)
        CA = [CA;ccn(q)-1,ccn(q)-(sidenum^2)-sidenum;
                 ccn(q)-1,ccn(q)-(sidenum^2)+sidenum;
                 ccn(q)-1,ccn(q)+(sidenum^2)+sidenum;
                 ccn(q)-1,ccn(q)+(sidenum^2)-sidenum;
                 ccn(q)+1,ccn(q)-(sidenum^2)-sidenum;
                 ccn(q)+1,ccn(q)-(sidenum^2)+sidenum;
...
function [C,uBasket,FBasket,Kout] = generateC(sel,NC,CA,A,E,C)
