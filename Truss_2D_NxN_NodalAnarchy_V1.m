% 2D NxN Square Truss Problem-- NODAL ANARCHY
...
sel = 0.01; % unit cube side length of 1 cm
r = 250*(10^-6); % Radius of 250 micrometers for cross-sectional 
...
% Input Member Position Array
% Case 1, 1 unit cell (3x3 grid)
MP = [];

% Generate nodal coordinate vector, connectivity array
[NC,CA] = generateNCCA(MP);
...
%----------%
% FUNCTION TO GENERATE NODAL COORDINATE VECTOR AND CONNECTIVITY ARRAY FROM
% MEMBER POSITION ARRAY
function [NC,CA] = generateNCCA(MP)
    NC = [];
    CA = [];
    %stuff...
end
