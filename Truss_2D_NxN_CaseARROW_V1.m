...
%plot(UC.^2,C11,'b*',UC.^2,C22,'r*',UC.^2,C21,'c*',UC.^2,C12,'m*',UC.^2,C33,'g*')
plot(UC.^2,C11,'b*',UC.^2,C12,'r*',UC.^2,C33,'g*')
xlabel('Number of Unit Cells');
ylabel('C-Value (Pa)');
%legend('C11','C22','C21','C12','C33');
...
function CA = generateCA(nucFac,sidenum)
    ...
    % State connections based on each central node 
    for i = 1:1:size(cn,1)
        CA = [CA;cn(i),cn(i)+1;cn(i),cn(i)-sidenum;cn(i),cn(i)+sidenum-1];
        CA = [CA;cn(i)-sidenum,cn(i)+sidenum-1;...
              cn(i)+1,cn(i)+sidenum-1];
    end
end
